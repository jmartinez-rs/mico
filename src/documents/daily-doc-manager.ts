...
export class DailyDocManager {
   ...
   */
  async appendCommitAnalysis(
    analysisResult: CommitAnalysisResult,
    language: "es" | "en" = "es"
  ): Promise<string> {
    ...
    if (!fileExists) {
      ...
      const header = isEn
        ? `# Development Report - ${dateStr} 🐒

> Document generated by **Mico**, the development observer agent.

## 📌 Daily Overview
This report documents tasks and changes made on **${dateStr}**.

## 📜 Commits and Deployments Log
`
        : `# Informe de Desarrollo - ${dateStr} 🐒
...
`;
    ...
    const timeLocale = isEn ? "en-US" : "es-ES";
    const timeStr = commit.date.toLocaleTimeString(timeLocale, {
      hour: "2-digit",
      minute: "2-digit",
      second: "2-digit",
    });
...
- **${isEn ? "Time" : "Hora"}:** \`${timeStr}\`
- **${isEn ? "Author" : "Autor"}:** ${commit.author}
- **Hash:** \`${commit.hash}\`
